...
import Form16Statement from './Form16view1';
import { useNetworkStatus } from '@/components/utils/network';

export default function Form16Viewer() {
  ...
  if (showForm16View) {
    return <Form16Statement fiscalYear={selectedFY} onBack={() => setShowForm16View(false)} />;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white p-8">
          <h2 className="text-3xl font-bold text-center">Form 16 Viewer</h2>
          ...
          <div className="mb-8">
            ...
            <div className="relative">
              ...
              >
                ...
                <ChevronDown size={22} className={`text-indigo-500 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                <div className="absolute z-10 mt-2 w-full bg-white border border-indigo-200 rounded-xl shadow-xl overflow-hidden">
                  {availableFY.map((fy) => (
                    <div
                      key={fy}
                      onClick={() => selectFY(fy)}
                      className={`px-5 py-4 text-gray-700 hover:bg-indigo-50 cursor-pointer transition-colors ${selectedFY === fy ? 'bg-indigo-100 text-indigo-700 font-medium' : ''}`}
                    >
                      {fy}
                    </div>
                  ))}
        ...
        <div className="bg-indigo-50 px-6 py-5 border-t border-indigo-100">
          <p className="text-sm text-indigo-700 text-center">
            Form 16 contains quarterly TDS/TCS details credited to your account
          </p>
        </div>
      </div>
    </div>
