...
export default function GstinDDORegistrationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate Area (optional but if provided, validate)
    setFieldErrors({}); // Clear previous errors
    if (formData.area && formData.area.trim() !== '') {
      console.log('Validating Area...', formData.area.length, 'characters');
      const areaValidation = validateName(formData.area, 'Area');
      if (!areaValidation.valid) {
        console.error('Area validation failed:', areaValidation.message);
        setFieldErrors({ area: areaValidation.message });
        toast.error('Area: ' + areaValidation.message);
        return;
      }
      console.log('Area validation passed');
    } else {
      console.log('Area is empty, skipping validation');
    }
    ...
    // Validate City (optional but if provided, validate)
  ...
  const columns = [
    ...
    { key: 'pinCode', label: 'Pincode', render: (val, row) => val || row.pinCode || '-' },
