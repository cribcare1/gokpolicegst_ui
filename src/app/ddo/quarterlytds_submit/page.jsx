...
export default function TDSQuarterlyCreate() {
  ...
  // INPUT HANDLER WITH REVISION RESET LOGIC
  const update = (e) => {
    ...
    if (["deducteeCount", "challanAmount", "taxDeducted"].includes(name)) {
      ...
      // Auto-reset revision if challan == tax deducted
      if (temp.challanAmount === temp.taxDeducted) {
        temp.revision = "";
      }
  ...
  const handleSubmit = async () => {
    try {
      const f = formData;
      const showRevisionField = f.challanAmount !== f.taxDeducted;
      ...
      // Validate revision ONLY if field is visible
      if (showRevisionField && !f.revision)
        return toast.show("Select Revision Filed", "error");
      ...
      toast.show(err.message || LOGIN_CONSTANT.SOMETHING_WENT_WRONG, "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const showRevisionField = formData.challanAmount !== formData.taxDeducted;
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-6 items-start">
          <div className="flex flex-col lg:flex-row items-start justify-between gap-4">
                      ...
                      <Button
                        variant="outline"
                        onClick={() => router.replace("/ddo/quarterly_tds_list")}
                      >
                        ← Back to List
                      </Button>
